...
function getConfig() {
  return {
    baseUrl: process.env.CONVIQ_BASE_URL?.replace(/\/$/, ""),
    accountId: process.env.CONVIQ_ACCOUNT_ID,
    apiKey: process.env.CONVIQ_API_KEY,
    inboxId: process.env.CONVIQ_WHATSAPP_INBOX_ID,
  }
}
...
async function conviqFetch<T>(path: string, options?: RequestInit): Promise<T | null> {
 ...
 */
async function createContact(name: string, phone: string): Promise<ConviqContact | null> {
 ...
 */
async function createContactInbox(contactId: number): Promise<string | null> {
...
export async function sendWhatsAppMessage(
  ...
  const sourceId = await createContactInbox(contact.id)
...
export async function notifyBuyerPaymentSuccess(
  ...
  await sendWhatsAppMessage(formattedPhone, name, message)
}
